...
-------------------------------------------*/
        ...
        System.out.println("treeSet = " + treeSet.descendingSet());
...
- get("유재석") 출력
- size() 출력
- remove("유재석") 후 map1 전체 출력
출력 예시(순서는 달라도 됨):
[get 유재석] 67
[size] 4
[remove 후 map1] {서장훈=100, 강호동=100, 신동엽=78}
-------------------------------------------*/
        Map<String,Integer> map1 = new HashMap<>();
...
-------------------------------------------*/
        ...
        for (String str : jsonAry){
            String[] strAry = str.split(":");
            map2.put(strAry[0], strAry[1]);
        }
...
-------------------------------------------*/
        ...
        map3.put("name", "삼성전자"); map3.put("price", "72000"); map3.put("volume", "1500000");
        Map<String, Object> map4 = new HashMap<>();
        map4.put("name", "카카오"); map4.put("price", "52000"); map4.put("volume", "800000");
        Map<String, Object> map5 = new HashMap<>();
        map5.put("name", "네이버"); map5.put("price", "210000"); map5.put("volume", "300000");
        stockList.add(map3);
        stockList.add(map4);
        stockList.add(map5);
        System.out.println("stockList = " + stockList);
        stockList.forEach((stock)-> {
            System.out.printf("종목명: %s / 가격: %s / 거래량: %s", stock.get("name"), stock.get("price"), stock.get("volume"));
        });
    }
}
