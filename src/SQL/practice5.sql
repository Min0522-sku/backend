...
select  p.제품명, pc.카테고리명 from pcategory pc inner join product p on 카테고리번호_pk = 카테고리번호_fk;
-- [문제 2] '노트북' 카테고리에 속하는 모든 제품의 제품명과 제품가격을 조회하세요. (조회결과: 2개 레코드)
select p.제품명, p.제품가격  from product p  inner join pcategory pc on p.카테고리번호_fk = pc.카테고리번호_pk where pc.카테고리명 = '노트북';
...
select p.제품명, s.재고수량, s.재고등록날짜 from product p inner join stock s on p.제품번호_pk = s.제품번호_fk where p.제품명 = '그램 15인치';
-- [문제 5] 모든 제품의 제품명, 카테고리명, 재고수량을 한 번에 조회하세요. (3개 테이블 조인) (조회결과: 8개 레코드)
select p.제품명, pc.카테고리명, s.재고수량 from pcategory pc join product p on pc.카테고리번호_pk = p.카테고리번호_fk join stock s on p.제품번호_pk = s.제품번호_fk;
-- [문제 6] 모든 카테고리의 카테고리명과 해당 카테고리에 속한 제품명을 조회하세요. 만약 카테고리에 속한 제품이 없더라도 카테고리명은 모두 표시되도록 하세요. (조회결과: 7개 레코드)
select * from pcategory left join product on 카테고리번호_pk = 카테고리번호_fk;
-- [문제 7] 재고가 한 번도 등록되지 않은 제품의 제품명을 조회하세요. (조회결과: 1개 레코드)
select * from product left join stock on 제품번호_pk = 제품번호_fk where 재고번호_pk is null;
