...
-- 테이블에서 그룹절 : group by 절
-- select 속성명 from 테이블명 group by 그룹기준
-- 주의할점: 그룹 기준외 다른 속성들의 대해 어떻게 보여줄지 정의 안됨. (조회 결과는 무조건 표/테이블)
...
-- 기본집계함수 
select sum(bamount) from buy; -- but 테이블내 bamount 속성값 들의 전체 합계
select avg(bamount) from buy; -- but 테이블내 bamount 속성값 들의 전체 평균
select min(bamount) from buy; -- but 테이블내 bamount 속성값 들의 전체중 최솟값
select max(bamount) from buy; -- but 테이블내 bamount 속성값 들의 전체중 최댓값
select count(bamount) from buy; -- but 테이블내 bamount 속성값 들의 전체 레코드 수(null 제외)
select count(*) from buy; -- but 테이블내 bamount 속성값 들의 전체 레코드 수 (null 포함)
...
-- having : 그룹절의 조건절, where: 그룹 전 조건 vs having : 그룹 후 조건
 -- 그룹 전 조건 where 
 select * from buy where bamount >3; -- 구매수량이 3이상인
 -- 그룹 후 조건 having
 select mid, sum(bamount * bprice) as 총구매금액 from buy group by mid having 총구매금액 > 1000;
 -- 총구매금액(그룹 후에 존재하는 속성)이 1000 이상인
 -- select 속성명 from 테이블명 where 일반조건절 group by 그룹기준 having 그룹조건절

-- order by : 정렬, asc 오름차순(기본값) ,desc 내림차순
-- 주의할점 : 다중정렬은 order by 1차정렬기준, 2차정렬기준
-- 1차 정렬후 동일한 값 끼리의 2차 정렬 실행
...
-- limit : 조회 결과 제한(검색/페이징) / limit 시작번호, 갯수;
select * from member limit 2; -- 조회 결과 위에서 2개만 조회
select * from member limit 0,2; -- 조회 결과에서 0(첫번째부터) 부터 2개 조회한다.
